...
- **[langgraph-crosschain](https://github.com/karthyick/langgraph-crosschain)** ![GitHub stars](https://img.shields.io/github/stars/karthyick/langgraph-crosschain?style=social) - LangGraph extension enabling direct cross-chain node communication, shared state, and message broadcasting across separate graphs. Enables modular multi-agent workflows where Chain1.NodeA can call Chain2.NodeB without merging into one monolithic graph. MIT licensed.
...
- **[Rubricon](https://github.com/karthyick/evaluation-first-attention)** ![GitHub stars](https://img.shields.io/github/stars/karthyick/evaluation-first-attention?style=social) - Specification-first generation framework. Produces evaluation rubrics *before* generation and uses them as conditioning targets through failure-weighted reattention (FWRL). Pluggable evaluators (LLM judge, regex, function, ensemble), backends (LiteLLM, vLLM, mock), and convergence policies. Paired with a research paper on Evaluation-First Attention. MIT licensed.
- **[distill-json](https://github.com/karthyick/DISTILL)** ![GitHub stars](https://img.shields.io/github/stars/karthyick/DISTILL?style=social) - Lossless JSON compression for LLM payloads. Reduces tokens 60-85% on bulk repeated records via schema extraction, dictionary encoding, and equivalence partitioning. Drop-in replacement for `json.dumps()` in OpenAI/Anthropic/LangChain workflows. MIT licensed.
...
- **[tracemaid](https://github.com/karthyick/tracemaid)** ![GitHub stars](https://img.shields.io/github/stars/karthyick/tracemaid?style=social) - Auto-generates Mermaid call graphs from OpenTelemetry traces for Python services. Visualizes microservice request paths and LLM agent workflows from existing OTel instrumentation — useful for sharing call graphs in design docs (Mermaid renders natively in GitHub/GitLab). MIT licensed.
- **[semantic-llm-cache](https://github.com/karthyick/prompt-cache)** ![GitHub stars](https://img.shields.io/github/stars/karthyick/prompt-cache?style=social) - Decorator-based semantic LLM response caching. Wrap any LLM call with `@semantic_cache` and skip 20-40% of API calls when prompts are semantically similar. Provider-agnostic. MIT licensed.
